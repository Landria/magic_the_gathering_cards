...
RSpec.describe MagicTheGatheringCards::Cards do
  ...
  it 'fetches card grouped by set' do
    VCR.use_cassette('cards') do
      expect(described_class.fetch.group_by(:set)).to eq(cards.sort_by { |attrs| attrs.set })
    end
  end

  it 'fetches card grouped by set and rarity' do
    VCR.use_cassette('cards') do
      set = described_class.fetch.group_by(:set, :rarity)
  ...
  it 'fetches card with only red AND blue colors' do
    VCR.use_cassette('cards') do
      set = described_class.fetch.reduce(colors: [:red, :blue])
  ...
  it 'fetches card with only white colors ' do
    VCR.use_cassette('cards') do
      set = described_class.fetch.reduce(colors: [:white])
  ...
  it 'fetches card with only types ["Host","Creature"] ' do
    VCR.use_cassette('cards') do
      set = described_class.fetch.reduce(types: [:host, :creature])
  ...
  it 'skips reduce on wrong attribute' do
    VCR.use_cassette('cards') do
      set = described_class.fetch.reduce(superTypes: [:host, :creature], colors: [:red])
  ...
  it 'fetches card with only red OR blue colors' do
    VCR.use_cassette('cards') do
      set = described_class.fetch.soft_reduce(colors: [:red, :blue])
  ...
  it 'fetches card with only colors red OR blue and manaCost with "{3}{U}"' do
    VCR.use_cassette('cards') do
      set = described_class.fetch.soft_reduce(colors: [:red, :blue], manaCost: '{3}{U}')
